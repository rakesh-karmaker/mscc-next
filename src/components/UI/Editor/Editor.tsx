...
export default function Editor({
  ...
  const renderAllMath = useCallback(() => {
    ...
    mathElements.forEach((el) => {
      try {
        ...
        // Render the formula
        window.katex.render(formula, el as HTMLElement, {
          throwOnError: false,
  ...
  const isContentEmpty = useCallback((element: HTMLElement) => {
  ...
  const handleContentChange = useCallback(() => {
    if (contentRef.current && register) {
      ...
      if (isEmpty) {
        register("content", { onChange: () => {} }).onChange({
          target: {
            name: "content",
            value: "",
          },
  ...
  const ensureParagraphAfterMath = useCallback((mathElement: HTMLElement) => {
    ...
    (e: KeyboardEvent) => {
      ...
      if (e.key === "Backspace") {
        ...
        if (prevNode && prevNode.nodeType === Node.ELEMENT_NODE) {
          ...
          if ((prevElement as HTMLElement).hasAttribute("data-math-formula")) {
