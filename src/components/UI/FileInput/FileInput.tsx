...
// const FileInput = ({ register, errors, children, name }: {
...
//       {errors && typeof errors.message === 'string' && <p className="error-message">{errors.message}</p>}
//     </div>
//   );
// };
...
const FileInput = ({
  ...
  return (
    <div className="file-input">
      <p className="input-heading">{children}</p>
      <input {...register(name)} id={name} type="file" accept="image/*" onChange={(e) => handleFileInput(e)} />
      <label ref={labelRef} htmlFor={name} className="highlighted-text file-label flex gap-2 items-center">
        <FaUpload /> <span>Upload File</span>
      </label>
